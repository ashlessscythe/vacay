...
export async function GET(request: Request) {
  try {
    ...
    if (!dateRegex.test(startDate) || !dateRegex.test(endDate)) {
      return new NextResponse("Invalid date format. Use YYYY-MM-DD", {
        status: 400,
      });
    }
    ...
    const formattedData = users.map((user) => ({
      ...
      leaves: user.leaves_leaves_user_idTousers.map((leave) => ({
        ...
        dateStart: new Date(
          leave.date_start.toLocaleString("en-US", {
            timeZone: currentUser.companies?.timezone || "America/Denver",
          })
        )
          .toISOString()
          .split("T")[0],
        dateEnd: new Date(
          leave.date_end.toLocaleString("en-US", {
            timeZone: currentUser.companies?.timezone || "America/Denver",
          })
        )
          .toISOString()
          .split("T")[0],
        status:
          leave.status === 1 ? ("approved" as const) : ("pending" as const),
...
export async function POST(request: Request) {
  try {
    ...
    const hasPermission =
      currentUser.admin ||
      (currentUser.manager &&
        (leave.users_leaves_user_idTousers.department_id ===
          currentUser.department_id ||
          supervisedDepartments.some(
            (d) =>
              d.department_id ===
              leave.users_leaves_user_idTousers.department_id
          )));
